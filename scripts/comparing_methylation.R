...
pacman::p_load(
  ...
  temp <- fread("//wsl$/nix/home/shei/methylation_phasing/development/methylation_phasing/results/ZymoFecal_contig_mmlong2_lite_u26020317ctg/split/read_clustering.tsv")
  ggplot(temp) +
    aes(
      x = GATC_5mC_3,
      y = CCAAG_6mA_3,
  ...
  cl <- hclust(
    dist(temp %>% select(-read_id, -cluster_id))
  )
  
  plot_raw <- temp %>%
    mutate(
      read_id = factor(read_id, levels = unique(read_id)[cl$order])
    ) %>% 
    pivot_longer(cols = -c("read_id", "cluster_id"))   %>% 
    ggplot() +
    aes(
      x = name,
      y = read_id,
    ...
    facet_grid(cluster_id~., scales = "free", space = "free") +
    theme(
  ...
  plot_raw
  temp <- fread("//wsl$/nix///home/shei/methylation_phasing/development/methylation_phasing/results/ZymoFecal_bin-mmlong2_lite.bin.1.1064_contig-mmlong2_lite_u26020317ctg_split/mmlong2_lite.bin.1.1064/read_clustering.tsv")
  ggplot(temp) +
    aes(
      x = GATC_5mC_3,
      y = GGCGCC_5mC_2,
  ...
  modkit_extract <- fread("//wsl$/nix/home/shei/methylation_phasing/development/methylation_phasing/results/ZymoFecal_bin2.474/contig_1330_modkit_extract.tsv")
